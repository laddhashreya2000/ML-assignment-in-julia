...
function costFunction(X, Y, B, rate)
    cost = sum(((X * B) - Y).^2) + (rate * sum(abs.(B)))
    return cost
end
rate = 0
...
for i in 30:129
    ...
    rmse[i-29] = sqrt(sum((YPred_val - Y_val).^2)/(m_train - m_val))
end
...
alpha = minimum_index(rmse) +29

rate = (alpha -100)*10000000
newB, costHistory = gradientDescent(X_train, Y_train, B, rate, learningRate, 1000)
